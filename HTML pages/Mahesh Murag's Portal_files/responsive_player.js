(function (jQuery) {
    ...
    jQuery.fn.slides_responsive = function (options) {
        // Default settings
        jQuery.fn.slides_responsive.defaults = {
            "width": "4"
        };

        var opts = jQuery.extend({}, jQuery.fn.slides_responsive.defaults, options);
        return this.each(function () {

            var added_pause = 10000,
                j_this = jQuery(this),
                magnify,
                aspectRatio = j_this.data("aspect-ratio"),
                j_holder = j_this.closest(".slideshow_holder"),
                holderWidth = j_this.width(),
                holderHeight = holderWidth / aspectRatio,
                cropWidth, cropHeight,//selection variables
                showConfig = j_this.data("show-config"),
                thisId = j_this.attr("id"),
                pager_dots = "",
                original_cropValues;
                transitionType = showConfig[0],
                dots = showConfig[1],
                speed = showConfig[2] * 1000,
                arrows = showConfig[3],
                j_images = j_this.find("img"),
                count = j_images.length,
                    counter=0,
                getResizeVals = function (w, cropValues) {
                    var m = w / cropValues[0];
                },
                resume_show = function(){
                    j_this.cycle('resume');
                },
                pause = function (){
                    j_this.cycle('pause');
                },
                j_holder.height(holderHeight),
                loadSlides= function(){
                    if (counter < count){
                        j_this = jQuery(j_images).eq(counter);
                        cropValues = j_this.data("crop-values");
                        consoleLog("j_this cropValues",cropValues)
                        original_cropValues = j_this.data("crop-values");
                        cropWidth = cropValues[0];
                        getResizeVals(holderWidth, cropValues)
                        imageWidth = j_this.width();
                        magnify = holderWidth / cropWidth;
                        checkSlide()
                    }
                    else
                    {
                ...
                checkSlide = function(){
                    if (j_this.attr("src").substring(0,26) =="/uimg/waiting_for_approval") {
                        thisSlide = j_this;
                        j_fullimage = jQuery("<img />").attr({
                            "src":j_this.attr("src") +"?hack="+ new Date().getTime(),
                            "id": "toCrop",
                            "visibility":"hidden"
                        }).load(function(){
                                jQuery("body").append(j_fullimage)
                                original_width =  j_fullimage.width();
                                consoleLog ("original_width",original_width)
                                if (original_width == 85){
                                    j_this.closest(".slide").addClass("pending_approval")
                                    thisSlide.attr({
                                        "src":j_this.attr("src") +"?hack="+ new Date().getTime(),
                                        "style":
                                            "width: 100%;" +
                                ...
                                {
                                    cropWidth = original_cropValues[0];
                                    magnify = holderWidth / cropWidth;
                                    for (var i = 0; i < cropValues.length; i++) {
                                        cropValues[i] = (original_cropValues[i] *= magnify);
                                    }
                                    thisSlide.attr({
                                        "src":j_this.attr("src") +"?hack="+ new Date().getTime(),
                ...
                finalizeSetup = function (){
                    consoleLog("finished cropping slides");
                    consoleLog("--holderHeight", holderWidth, "*", aspectRatio, holderWidth * aspectRatio, holderHeight);
                    if (arrows == "true") {
                        opts.prev = "#" + thisId + "_prev";
                        opts.next = "#" + thisId + "_next";
                        jQuery(opts.prev).show();
                        jQuery(opts.next).show();
                        jQuery(opts.next).click(function(){
                            consoleLog ("pause show")
                            pause();
                            timeout = setTimeout(function(){
                                    resume_show()}, added_pause
                            );
                        });
                    ...
                    if (transitionType == "flip")  transitionType = "flipVert";
                    if (transitionType == "tiles")  transitionType = "tileSlide";
                    jQuery("#" + thisId).cycle({
                        fx: transitionType,//
                        timeout: speed,
                        prev: opts.prev,
                    ...
                    pager_dots = "#" + thisId + "_dots";
                    jQuery(pager_dots).hide();
                    if (dots == "true") {
                        jQuery(pager_dots).show();
                        jQuery(pager_dots).click(function(){
                            consoleLog ("pause show")
                            pause();
                            timeout = setTimeout(function(){
                                    resume_show()}, added_pause
            ...
            jQuery(document).on("click", ".slide", function () {
                if (jQuery(this).attr("href")) window.location = jQuery(this).attr("href");
            })
        });
    }
})(jQuery);
...
(function (e) {
    ...
    e.fn.cycle = function (i) {
        ...
        window.console && consoleLog && consoleLog("[cycle2] " + Array.prototype.join.call(arguments, " "))
...
(function (a) {
    "use strict", a.fn.cycle.transitions.shuffle = {transition: function (b, c, d, e, f) {
        function l(a) {
            this.stack(b, c, d, e), a()
        }

        a(d).show();
        var g = b.container.css("overflow", "visible").width(), h = b.speed / 2, i = e ? c : d;
        b = b.API.getSlideOpts(e ? b.currSlide : b.nextSlide);
        var j = {left: -g, top: 15}, k = b.slideCss || {left: 0, top: 0};
        b.shuffleLeft !== undefined ? j.left = j.left + parseInt(b.shuffleLeft, 10) || 0 : b.shuffleRight !== undefined && (j.left = g + parseInt(b.shuffleRight, 10) || 0), b.shuffleTop && (j.top = b.shuffleTop), a(i).animate(j, h, b.easeIn || b.easing).queue("fx", a.proxy(l, this)).animate(k, h, b.easeOut || b.easing, f)
    }, stack: function (b, c, d, e) {
        var f, g;
        if (e)b.API.stackSlides(d, c, e), a(c).css("zIndex", 1); else {
            g = 1;
            for (f = b.nextSlide - 1; f >= 0; f--)a(b.slides[f]).css("zIndex", g++);
            for (f = b.slideCount - 1; f > b.nextSlide; f--)a(b.slides[f]).css("zIndex", g++);
            a(d).css("zIndex", b.maxZ), a(c).css("zIndex", b.maxZ - 1)
        }
    }}
})(jQuery);
...
/*! Plugin for Cycle2; Copyright (c) 2012 M. Alsup; ver: 20121120 */
(function (a) {
    "use strict", a.fn.cycle.transitions.scrollVert = {before: function (a, b, c, d) {
        a.API.stackSlides(a, b, c, d);
        var e = a.container.css("overflow", "hidden").height();
        a.cssBefore = {top: d ? -e : e, left: 0, opacity: 1, display: "block"}, a.animIn = {top: 0}, a.animOut = {top: d ? e : -e}
    }}
})(jQuery);
